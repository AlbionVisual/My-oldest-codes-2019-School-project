...
int main()
{
	...
	for (int i = 1; i < chainCount + 1; i++)
	{
		...
		{
			for (int k = 0; k < requiredFormuls[i].numOfDesignations; k++) {
				if (requiredFormuls[i].formulasValues[k].isInDirectRatio) cout << " + " << requiredFormuls[i].formulasValues[k].designation;
				else cout << " - " << requiredFormuls[i].formulasValues[k].designation;
			};
			int sum = 0;
			for (int k = 0; k < requiredFormuls[i].numOfDesignations; k++) {
				if (requiredFormuls[i].formulasValues[k].isInDirectRatio) sum += requiredFormuls[i].formulasValues[k].designation;
				else sum -= requiredFormuls[i].formulasValues[k];
			};

			values[n + chainCount - i].value = sum;
			values[n + chainCount - i].measure = "mpss";

			cout << " = " << sum << ' ' << values[n+chainCount - i].measure;
			cout << endl;
		}
		else
		{
